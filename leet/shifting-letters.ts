...
function shiftingLetters(s: string, shifts: number[][]): string {
    ...
    console.log('prefix arr: ', prefixShiftArray.join(' , '));
    for (let i = prefixShiftArray.length - 2; i >= 0; i--) {
        console.log(`${i}:`, s[i], buffer[i], diff);
...
const testCases = [
    {
        name: '1',
        inputStr: 'abc',
        inputShifts: [[0, 1, 0], [1, 2, 1], [0, 2, 1]],
        expected: 'ace',
    },
    {
        name: '2',
        inputStr: 'dztz',
        inputShifts: [[0, 0, 0], [1, 1, 1]],
        expected: 'catz',
    },
];

testCases.forEach(({ name, inputStr, inputShifts, expected }) => {
    Deno.test(name, () => {
        const result = shiftingLetters(inputStr, inputShifts);
